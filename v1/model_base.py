...
class MidiNet(object):
    ...
    def build_model(self):
        ...
        self.G = self.generator(self.z, prev_x=self.prev_bar)
        self.D, self.D_logits, self.fm = self.discriminator(self.images,  reuse=False)

        self.sampler = self.sampler(self.z,   prev_x=self.prev_bar)
        ...
        self.d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(self.D_logits + 1e-8, 0.9*tf.ones_like(self.D)))
        self.d_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(self.D_logits_ + 1e-8, tf.zeros_like(self.D_)))
        self.g_loss0 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(self.D_logits_ + 1e-8, tf.ones_like(self.D_)))
    ...
    def train(self, config):
        ...
        if config.dataset == 'MidiNet_v1':
            ...
            data_X = np.load(np_dir + 'truncated_midi100.npy') #Shape: (n, 1, 16, 128), where n is the number of measures(bars) of training data.
            prev_X = np.load(np_dir + 'prev_truncated_midi100.npy') #Shape: (n, 1, 16, 128), if the bar is a first bar, it's previous bar = np.zeros(1,16,128)
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                batch_labels = None # data_y[idx*config.batch_size:(idx+1)*config.batch_size]
                '''
                ...
                discrim = self.D.eval({self.images: batch_images, self.z: batch_z, self.prev_bar:prev_batch_images })
                counter += 1
                print("Epoch: [%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f, discrim_val: %f" \
                    % (epoch, idx, batch_idxs,
                        time.time() - start_time, errD_fake+errD_real, errG, np.mean(discrim)))
    ...
    def generator(self, z, y=None, prev_x = None):
        ...
        if not self.y_dim:
            ...
            h2 = tf.nn.relu(self.g_bn2(
                deconv2d(h1, [self.batch_size, 4, 1, self.gf_dim * 2], k_h=2, k_w=1, d_h=2, d_w=2, name='g_h2')))
            h2 = conv_prev_concat(h2, h2_prev)

            h3 = tf.nn.relu(self.g_bn3(
                deconv2d(h2, [self.batch_size, 8, 1, self.gf_dim * 2], k_h=2, k_w=1, d_h=2, d_w=2, name='g_h3')))
