...
class MidiNet(object):
    ...
    def train(self, config):
        ...
        if config.dataset == 'MidiNet_v1':
            ...
            data_X = np.load(
                np_dir + 'truncated_midi.npy')  # Shape: (n, 1, 16, 128), where n is the number of measures(bars) of training data.
            prev_X = np.load(
                np_dir + 'prev_truncated_midi.npy')  # Shape: (n, 1, 16, 128), if the bar is a first bar, it's previous bar = np.zeros(1,16,128)
